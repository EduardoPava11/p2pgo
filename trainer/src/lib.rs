...
impl GoDataset {
        ...
        if path.exists() && path.is_dir() {
            for entry in std::fs::read_dir(path)? {
                ...
                if file_path.is_file() && file_path.extension().map_or(false, |ext| ext == "cbor") {
                    ...
                    let score_proof_data = file_data
                        .iter()
                        .enumerate()
                    ...
                    if let Some(score_data) = score_proof_data {
                        if let Ok(score_proof) = serde_cbor::from_slice::<
                            p2pgo_core::value_labeller::ScoreProof,
                        >(&score_data[1..])
                        {
                            ...
                            if matches!(
                                ...
                                p2pgo_core::value_labeller::ScoringMethod::Territory
                                    | p2pgo_core::value_labeller::ScoringMethod::Area
                            ) {
                                ...
                                tracing::warn!(
                                    "Skipping game with non-territory/area scoring: {}",
                                    file_path.display()
                                );
                            }
                        }
                        ...
                        tracing::warn!(
                            "Skipping game without score proof: {}",
                            file_path.display()
                        );
                    }
                }
    ...
    pub fn get_batch<B: Backend>(
        ...
        batch_size: usize,
        device: &B::Device,
    ) -> (Tensor<B, 2>, Tensor<B, 1, Int>, Tensor<B, 1>) {
...
pub fn load_games_from_dir<P: AsRef<Path>>(
    path: P,
) -> Result<GoDataset, Box<dyn std::error::Error>> {
    let mut samples = Vec::new();
    let path = path.as_ref();
    ...
    for entry in std::fs::read_dir(path)? {
        ...
        if file_path.is_file() && file_path.extension().map_or(false, |ext| ext == "cbor") {
            ...
            if score_proof_data.is_none() {
                tracing::warn!(
                    "Skipping game file without score proof: {}",
                    file_path.display()
                );
                continue;
            }
