...
impl E2ETestRunner {
    ...
    fn run_ai_integration_test(&mut self) -> anyhow::Result<()> {
        ...
        self.ui_tx.send(UiToNet::MakeMove {
            mv: first_move.clone(),
            board_size: None,
        })?;
    ...
    #[cfg(feature = "headless")]
    fn wait_for_view_transition(
        &mut self,
        expected_view_prefix: &str,
        timeout_ms: u64,
    ) -> anyhow::Result<()> {
        ...
        while start.elapsed() < Duration::from_millis(timeout_ms) {
            ...
            if tick_count % 10 == 0 {
                println!(
                    "  Wait tick {}: view = {} (waiting for {})",
                    tick_count, current_view, expected_view_prefix
                );
            }
        ...
        anyhow::bail!(
            "Timeout waiting for view transition to '{}'. Current view: '{}'. Total ticks: {}",
            expected_view_prefix,
            self.app.get_current_view_debug(),
            tick_count
        )
    }
    ...
    fn wait_and_process(&mut self, duration_ms: u64) {
        ...
        println!(
            "  Final after {}ms: view = {}, total ticks = {}",
            duration_ms,
            self.app.get_current_view_debug(),
            tick_count
        );
...
fn test_ai_message_types() {
    ...
    let ghost_response =
        NetToUi::GhostMoves(vec![Coord::new(3, 3), Coord::new(4, 4), Coord::new(5, 5)]);
    ...
    match ghost_response {
        NetToUi::GhostMoves(coords) => {
            ...
            println!(
                "GhostMoves response type works with {} coordinates",
                coords.len()
            );
        }
...
fn test_ai_integration_compilation() {
    ...
    let coords = vec![Coord::new(3, 3), Coord::new(4, 4), Coord::new(5, 5)];
