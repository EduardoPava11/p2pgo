...
use p2pgo_core::value_labeller::{ScoringMethod, ScoreProof};
...
fn scoring_pipeline_works_correctly() {
    ...
    assert_eq!(score_proof.final_score, 
        (score_proof.territory_black as i16 + score_proof.captures_black as i16) as i16 - 
        ((score_proof.territory_white as i16 + score_proof.captures_white as i16) as f32 + score_proof.komi).round() as i16
    );
    ...
    assert!(training_data.len() > 10, "Should have generated meaningful training data");
    
    // Should contain at least one 'S' marker for ScoreProof
    assert!(training_data.iter().any(|&b| b == b'S'), "Training data should contain score proof marker");
    
    // Should contain at least one 'M' marker for MoveRecords
    assert!(training_data.iter().any(|&b| b == b'M'), "Training data should contain move records marker");
    ...
    for i in 0..game_state.moves.len() {
        if let Some(label) = labeller.get_value_label(i as u32) {
            ...
            assert!(label.game_outcome != 0.0, "Game outcome should be set for each move");
        } else {
    ...
    let deserialized: ScoreProof = serde_cbor::from_slice(&serialized).expect("Deserialization should work");
