...
use p2pgo_ui_egui::ui_config::{UiConfig, TerritoryMarkerType, SerializableColor};
use egui::Color32;
...
fn main() {
    ...
    println!("   Grid color: RGB({},{},{})", 
             config.board.grid_color.r,
             config.board.grid_color.g, 
             config.board.grid_color.b);
    ...
    for (name, tensors) in tensor_sets {
        ...
        println!("      Stone radius ratio: {:.3}", config.board.stone_radius_ratio);
        println!("      Territory marker ratio: {:.3}", config.territory.marker_size_ratio);
    }
        ...
        Ok(_) => {
            ...
            match UiConfig::load_from_file(config_path) {
                Ok(loaded) => {
                    ...
                    println!("      Button corner radius: {}", loaded.button.corner_radius);
                }
