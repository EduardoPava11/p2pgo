...
use p2pgo_core::{Move, GameState, GameEvent, Color, Coord};
...
impl GameActivityLogger {
    ...
    pub fn log_game_start(&self, game_id: &str, board_size: u8, our_color: Option<Color>) {
        self.log_entry(ActivityType::GameStarted {
            board_size,
            our_color,
            opponent_id: None,
    ...
    pub fn log_move_made(&self, game_id: &str, mv: &Move, game_state: &GameState, thinking_time_ms: u32) {
        let move_data = self.move_to_data(mv, game_state.moves.len() as u32, Some(thinking_time_ms));
        let state_snapshot = self.snapshot_game_state(game_state);
        
        self.log_entry(ActivityType::MoveMade {
            move_data,
    ...
    pub fn log_move_received(&self, game_id: &str, mv: &Move, move_number: u32, latency_ms: Option<u32>) {
        let move_data = self.move_to_data(mv, move_number, None);
        
        self.log_entry(ActivityType::MoveReceived {
            move_data,
            latency_ms,
        }, Some(game_id.to_string()));
    }
    ...
    pub fn log_network_op(&self, op: NetworkOperation, success: bool, error: Option<String>) {
        self.log_entry(ActivityType::NetworkOp {
            operation: op,
            success,
            error,
        }, None);
    ...
    pub fn log_ui_interaction(&self, action: &str, target: &str, details: Option<serde_json::Value>) {
        self.log_entry(ActivityType::UiInteraction {
            interaction: UiInteraction {
    ...
    /// Log game end
    pub fn log_game_end(&self, game_id: &str, result: &str, final_score: Option<(f32, f32)>, total_moves: u32) {
        self.log_entry(ActivityType::GameEnded {
    ...
    pub fn log_error(&self, component: &str, error: &str, context: Option<serde_json::Value>) {
        self.log_entry(ActivityType::Error {
    ...
    pub fn log_performance(&self, metric_type: &str, value: f64, unit: &str, context: Option<&str>) {
        self.log_entry(ActivityType::Performance {
            metric: PerformanceMetric {
    ...
    fn snapshot_game_state(&self, state: &GameState) -> GameStateSnapshot {
        let black_stones = state.board.iter().filter(|&&c| c == Some(Color::Black)).count();
        let white_stones = state.board.iter().filter(|&&c| c == Some(Color::White)).count();
    ...
    fn print_entry(&self, entry: &GameActivityEntry) {
        ...
        match &entry.entry_type {
            ActivityType::MoveMade { move_data, .. } => {
                println!("[{}] MOVE: {} at {:?}", timestamp, move_data.move_type, move_data.coord);
            }
            ActivityType::NetworkOp { operation, success, error } => {
                if *success {
                    println!("[{}] NET: {} OK", timestamp, operation.op_type);
                } else {
                    println!("[{}] NET: {} FAILED - {:?}", timestamp, operation.op_type, error);
                }
            }
            ActivityType::Error { component, error, .. } => {
