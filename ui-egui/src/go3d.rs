...
use egui::{Color32, Context, Painter, Pos2, Rect, Stroke, Ui, Vec2};
...
impl Go3DGame {
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    // Title and controls
                    ui.label(
                        egui::RichText::new("3D Go - Three Intersecting Planes")
                            .size(24.0)
                            .color(Color32::from_gray(51)),
                    );
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .selectable_label(
                                matches!(self.current_view, ViewPlane::XY),
                                "XY Plane (Z=4)",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                matches!(self.current_view, ViewPlane::XZ),
                                "XZ Plane (Y=4)",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                matches!(self.current_view, ViewPlane::YZ),
                                "YZ Plane (X=4)",
                            )
                            .clicked()
                        {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.button("Random Stones").clicked() {
                            ...
                            for _ in 0..10 {
                                // XY plane
                                let coord1 =
                                    Coord3D::new(rng.gen_range(0..9), rng.gen_range(0..9), 4);
                                ...
                                // XZ plane
                                let coord2 =
                                    Coord3D::new(rng.gen_range(0..9), 4, rng.gen_range(0..9));
                                ...
                                // YZ plane
                                let coord3 =
                                    Coord3D::new(4, rng.gen_range(0..9), rng.gen_range(0..9));
    ...
    fn draw_3d_overview(&self, ui: &mut Ui, size: f32) {
        ui.group(|ui| {
            ...
            let xy_alpha = if matches!(self.current_view, ViewPlane::XY) {
                80
            } else {
                40
            };
            painter.rect_filled(
                Rect::from_center_size(center, Vec2::new(plane_size * 0.7, plane_size * 0.7)),
                0.0,
                Color32::from_rgba_unmultiplied(255, 100, 100, xy_alpha),
            );

            // XZ plane (at Y=4) - vertical front-back
            let xz_alpha = if matches!(self.current_view, ViewPlane::XZ) {
                80
            } else {
                40
            };
            ...
            let yz_alpha = if matches!(self.current_view, ViewPlane::YZ) {
                80
            } else {
                40
            };
            ...
            for (coord, color) in &self.board {
                let pos =
                    self.project_to_3d_overview(coord, center, plane_size * 0.7, offset * 0.3);
    ...
    fn draw_plane_intersections(&self, painter: &Painter, rect: Rect, cell_size: f32) {
        match self.current_view {
            ...
            ViewPlane::XZ => {
                ...
                painter.line_segment(
                    [Pos2::new(rect.min.x, z_pos), Pos2::new(rect.max.x, z_pos)],
            ...
            ViewPlane::YZ => {
                ...
                painter.line_segment(
                    [Pos2::new(y_pos, rect.min.y), Pos2::new(y_pos, rect.max.y)],
                    Stroke::new(2.0, Color32::from_rgb(100, 100, 255)),
                );
            }
        }
