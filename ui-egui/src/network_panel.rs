...
#[cfg(feature = "iroh")]
use p2pgo_network::relay_monitor::{RelayHealth, RelayHealthEvent, RelayHealthStatus, RelayStats};
...
impl NetworkPanel {
    pub fn new() -> Self {
        Self {
            ...
            is_relay_node: false,
            relay_mode: p2pgo_network::relay_config::RelayMode::Normal {
                max_reservations: 25,
                max_circuits: 50,
            },
    ...
    #[cfg(feature = "iroh")]
    pub fn set_relay_stats(
        &mut self,
        relay_stats: Arc<tokio::sync::RwLock<HashMap<String, RelayStats>>>,
    ) {
    ...
    fn draw_content(&mut self, ui: &mut egui::Ui) -> Option<crate::msg::UiToNet> {
        ui.horizontal(|ui| {
            ui.selectable_value(
                &mut self.show_diagnostics_tab,
                DiagnosticsTab::Overview,
                "Overview",
            );
            ui.selectable_value(
                &mut self.show_diagnostics_tab,
                DiagnosticsTab::ConnectionQuality,
                "Connection Quality",
            );
            ui.selectable_value(
                &mut self.show_diagnostics_tab,
                DiagnosticsTab::GameNetwork,
                "Game Network",
            );
            ui.selectable_value(
                &mut self.show_diagnostics_tab,
                DiagnosticsTab::Troubleshooting,
                "Troubleshooting",
            );
        });
    ...
    fn draw_connection_quality(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.add(
                egui::ProgressBar::new(quality_score)
                    .fill(quality_color)
                    .desired_width(200.0)
        ...
        egui::Grid::new("quality_metrics")
            ...
            .show(ui, |ui| {
                ...
                ui.label(if self.connection_quality.avg_latency < 100.0 {
                    "🟢"
                } else if self.connection_quality.avg_latency < 200.0 {
                    "🟠"
                } else {
                    "🔴"
                });
                ...
                ui.label(format!(
                    "{:.1} / {:.1} ms",
                    if self.connection_quality.min_latency == f32::MAX {
                        0.0
                    } else {
                        self.connection_quality.min_latency
                    },
                ...
                ui.label(if self.connection_quality.jitter < 20.0 {
                    "🟢"
                } else if self.connection_quality.jitter < 50.0 {
                    "🟠"
                } else {
                    "🔴"
                });
                ...
                ui.label(if self.connection_quality.packet_loss < 0.01 {
                    "🟢"
                } else if self.connection_quality.packet_loss < 0.05 {
                    "🟠"
                } else {
                    "🔴"
                });
                ...
                ui.label(if self.connection_quality.stability > 0.9 {
                    "🟢"
                } else if self.connection_quality.stability > 0.7 {
                    "🟠"
                } else {
                    "🔴"
                });
    ...
    fn draw_game_network(&mut self, ui: &mut egui::Ui) {
        ...
        ui.collapsing("Network Statistics", |ui| {
            ...
            let synced_games = self
                .game_connections
                .values()
                .filter(|g| g.is_synced)
                .count();
            let avg_move_latency = self
                .game_connections
                .values()
                .filter_map(|g| g.move_latency)
                .fold(0.0, |acc, l| acc + l)
                / total_games.max(1) as f32;
    ...
    fn draw_troubleshooting(&mut self, ui: &mut egui::Ui) -> Option<crate::msg::UiToNet> {
        ...
        ui.group(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    .add_enabled(test_enabled, egui::Button::new("Run Connection Test"))
                    .clicked()
                {
                    self.connection_test_in_progress = true;
                    self.test_results.clear();
                    self.test_results
                        .push("Starting connection test...".to_string());
        ...
        ui.group(|ui| {
            ...
            if ui.button("Export Diagnostics").clicked() {
                ...
                ui.ctx()
                    .output_mut(|o| o.copied_text = self.generate_diagnostic_report());
            }
    ...
    pub fn network_state_color(&self) -> egui::Color32 {
        use p2pgo_core::color_constants::{f32_to_u8_rgb, network_status};
    ...
    pub fn draw_network_badge(&self, ui: &mut egui::Ui) {
        ...
        let (rect, _) = ui.allocate_exact_size(
            egui::Vec2::new(radius * 2.0, radius * 2.0),
            egui::Sense::hover(),
        );
    ...
    fn get_network_tooltip(&self) -> String {
        ...
        if let Some(status) = &self.relay_health_status {
            ...
            if self.network_state == NetworkState::StartingRelay
                || self.network_state == NetworkState::Syncing
            {
...
impl ConnectionGraph {
    ...
    fn add_node(&mut self, id: &str) {
        if !self.node_indices.contains_key(id) {
            ...
            self.layout.insert(
                idx,
                egui::pos2(150.0 + radius * angle.cos(), 150.0 + radius * angle.sin()),
            );
        }
    }
    ...
    fn show(&self, ui: &mut egui::Ui) {
        let (response, painter) =
            ui.allocate_painter(egui::Vec2::new(300.0, 300.0), egui::Sense::hover());
