...
impl DualHeatMap {
    ...
    pub fn render_overlay(
        ...
        for y in 0..19 {
            for x in 0..19 {
                let sword_prob = if self.sword_enabled { sword_predictions[y][x] } else { 0.0 };
                let shield_prob = if self.shield_enabled { shield_predictions[y][x] } else { 0.0 };
    ...
    pub fn render_controls(&mut self, ui: &mut Ui) {
        ui.group(|ui| {
            ...
            if self.sword_enabled || self.shield_enabled {
                ...
                ui.horizontal(|ui| {
                    ui.label("Blend:");
                    ui.selectable_value(&mut self.blend_mode, BlendMode::Interference, "Interference");
                    ui.selectable_value(&mut self.blend_mode, BlendMode::Additive, "Additive");
                    ui.selectable_value(&mut self.blend_mode, BlendMode::Multiplicative, "Multiply");
                });
