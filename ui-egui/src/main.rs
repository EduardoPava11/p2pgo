...
fn main() -> anyhow::Result<()> {
    ...
    let _worker_handle =
        worker::spawn_worker(net_rx, net_tx.clone(), board_size, player_name.clone())?;

    // If ticket is provided, connect on startup
    if let Some(ticket_str) = ticket {
        // Short delay to allow worker to initialize
        std::thread::sleep(std::time::Duration::from_millis(500));
        let _ = net_tx.send(msg::NetToUi::Debug(format!(
            "Connecting via ticket: {}",
            &ticket_str
        )));
    ...
    eframe::run_native(
        "P2P Go",
        options,
        Box::new(move |_cc| Box::new(App::new(ui_tx, ui_rx, board_size, player_name))),
    )
