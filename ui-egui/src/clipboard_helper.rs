...
impl ClipboardHelper {
    ...
    pub fn copy_ticket(
        &mut self,
        ticket: &str,
        toast_manager: &mut crate::toast_manager::ToastManager,
    ) -> Result<bool> {
            ...
            if let Some(p2p_index) = addr.rfind("/p2p/") {
                ...
                let port = if addr.contains("/tcp/") {
                    if let Some(tcp_index) = addr.find("/tcp/") {
                        let port_end = addr[tcp_index + 5..]
                            .find('/')
                            .unwrap_or(addr[tcp_index + 5..].len());
                    ...
                    if let Some(udp_index) = addr.find("/udp/") {
                        let port_end = addr[udp_index + 5..]
                            .find('/')
                            .unwrap_or(addr[udp_index + 5..].len());
                ...
                if let Some(port_str) = port {
                    return format!(
                        "{} ({}:{}) [{}]",
                        address_type, "...", port_str, shortened_peer_id
                    );
                } else {
