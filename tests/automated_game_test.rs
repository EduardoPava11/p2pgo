...
fn main() {
    ...
    for (i, (x, y, expected_color)) in moves.iter().enumerate() {
        ...
        if game.current_player != *expected_color {
            println!("❌ Turn mismatch at move {}: expected {:?}, but it's {:?}'s turn", 
                     i + 1, expected_color, game.current_player);
            break;
        }
        
        let mv = Move::Place { x: *x, y: *y, color: *expected_color };
        match game.apply_move(mv) {
            Ok(_) => {
                if (i + 1) % 10 == 0 {
                    println!("  Move {}: {:?} plays at ({}, {})", i + 1, expected_color, x, y);
                }
            }
            Err(e) => {
                println!("❌ Move {} failed: {:?} at ({}, {}): {}", i + 1, expected_color, x, y, e);
                break;
            }
        }
    }
        ...
        Ok(path) => {
            ...
            match p2pgo_core::archiver::read_game_archive(&path) {
                Ok(restored) => {
                    ...
                    println!("- Can be used with: cargo run --bin train_neural -- {:?}", path);
                }
