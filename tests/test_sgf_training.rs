...
mod tests {
    ...
    async fn test_sgf_to_cbor_conversion() {
        ...
        assert!(
            file_size > 100,
            "CBOR file is too small: {} bytes",
            file_size
        );

        println!(
            "✅ SGF to CBOR conversion successful. File size: {} bytes",
            file_size
        );
    }
    ...
    async fn test_training_pipeline() {
        ...
        match result {
            Ok(training_results) => {
                ...
                println!(
                    "   Sword accuracy: {:.1}%",
                    training_results.sword_accuracy * 100.0
                );
                println!(
                    "   Shield accuracy: {:.1}%",
                    training_results.shield_accuracy * 100.0
                );
                println!("   Total examples: {}", training_results.total_examples);

                assert!(
                    training_results.total_examples > 0,
...
;B[ee];W[eg];B[ce];W[cg])"#;
        ...
        println!(
            "✅ SGF parsing successful. Parsed {} moves",
