...
async fn test_two_player_game_flow() {
    ...
    let game_channel2 = GameChannel::join(node2.clone(), game_id.to_string(), node1.node_id())
        .await
        .unwrap();
...
async fn test_relay_mode_switching() {
    use p2pgo_network::relay_config::{RelayConfig, RelayMode};
...
async fn test_network_recovery() {
    ...
    let game_channel1 = GameChannel::create(node1.clone(), game_id.to_string(), 9)
        .await
        .unwrap();
...
async fn test_high_throughput_moves() {
    ...
    let game_channel2 = GameChannel::join(node2.clone(), "perf-test".to_string(), node1.node_id())
        .await
        .unwrap();
