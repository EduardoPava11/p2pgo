...
async fn test_e2e_multiplayer_game() -> anyhow::Result<()> {
    let moves = vec![
        ...
        (Color::White, Move::Pass), (Color::Black, Move::Pass),
    ];
    let lobby = Lobby::new();
    let game_id = lobby.create_game(Some("E2E Test".to_string()), 9, false).await?;
    ...
    assert_eq!(serde_json::to_string(&state_p1)?, serde_json::to_string(&state_p2)?);
    ...
    assert_eq!(serde_json::to_string(&final_p1)?, serde_json::to_string(&final_p2)?);
