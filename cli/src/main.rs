...
fn ensure_logging_initialized() -> Result<()> {
    let mut result = Ok(());
    LOGGER_INIT.call_once(|| match init_logging() {
        Ok(_) => {}
        Err(e) => {
            result = Err(e);
        }
...
async fn main() -> Result<()> {
    ...
    if let Some(ticket) = args.ticket.as_ref() {
        ...
        if games.is_empty() {
            ...
            if let Some(game) = games.first() {
                ...
                // Get current state
                let game_state = channel
                    .get_latest_state()
                    .await
                    .ok_or_else(|| anyhow!("Failed to get current game state"))?;

                // Run the game loop
                return run_game_loop(game_state, channel, lobby, game.id.clone(), args.debug)
                    .await;
            }
        }
    }
    ...
    if args.spectator {
        ...
        let ticket = iroh_ctx.ticket();
        println!(
            "Spectator seed node ticket (share with players):\n{}",
            ticket
        );
    ...
    let role = if args.ticket.is_some() {
        ...
        args.role
            .ok_or_else(|| anyhow!("Role is required (--role host or --role join)"))?
    };
    ...
    match role {
        ...
        Role::Join => {
            ...
            let game_id = args
                .game_id
                .ok_or_else(|| anyhow!("Game ID is required when joining a game"))?;
...
async fn run_game_loop(
    ...
    loop {
        println!(
            "\n{:?} to move. Enter a move (e.g., 'D4'), 'pass', or 'resign':",
    ...
    if input.len() >= 2 {
        ...
        if col < board_size && row < board_size {
            return Ok(Move::Place {
                ...
                y: row,
                color: Color::Black,
            }); // Color will be overridden by the game
        }
    }

    Err(anyhow!(
        "Invalid move format. Examples: 'D4', 'pass', 'resign'."
    ))
}
...
fn print_game_state(game_state: &GameState) {
    println!(
        "\nBoard size: {}x{}",
        game_state.board_size, game_state.board_size
    );
...
mod tests {
    ...
    fn test_parse_move() {
        ...
        assert!(
            matches!(parse_move("D4", 19).unwrap(), Move::Place(coord) if coord.x == 3 && coord.y == 3)
        );
