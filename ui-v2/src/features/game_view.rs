...
use crate::core::{danger_button, primary_button, secondary_button, Card, Colors, Spacing, Styles};
        ...
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                Card::new().show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.label("Turn:");
                        let turn_text = if self.is_our_turn {
                            "Your turn"
                        } else {
                            "Opponent's turn"
                        };
                        let turn_color = if self.is_our_turn {
                            Colors::SUCCESS
                        } else {
            ...
            ui.vertical(|ui| {
                ...
                let heat_map = if self.show_heat_map {
                    ...
                    for pred in predictions {
                        if pred.coord.x < 9 && pred.coord.y < 9 {
                            heat_map[pred.coord.y as usize][pred.coord.x as usize] =
                                pred.probability;
                        }
                    }
                    Some(heat_map)
                ...
                let board_response = board_widget.interactive(self.is_our_turn).show(ui);

                if let Some(pos) = board_response.clicked_position {
                    if self.is_our_turn {
                        // Check if position is empty
                        let idx =
                            (pos.y as usize) * (self.game.board_size as usize) + (pos.x as usize);
