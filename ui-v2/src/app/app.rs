...
use crate::core::{apply_theme, ghost_button, Colors, Spacing};
use crate::features::{GameAction, GameView, LobbyAction, LobbyView, TrainingAction, TrainingView};
...
impl P2PGoApp {
    ...
    fn render_content(&mut self, ctx: &egui::Context) {
        CentralPanel::default().show(ctx, |ui| match self.router.current() {
            View::Lobby => {
    ...
    fn handle_lobby_action(&mut self, action: LobbyAction) {
        match action {
            LobbyAction::CreateGame(_) => {
                ...
                let _ = self.net_tx.send(UiToNet::CreateGame {
            ...
            match action {
                GameAction::PlaceStone(pos) => {
                    if game_view.is_our_turn {
                        ...
                        if game_view
                            .game
                            .apply_move(p2pgo_core::Move::Place {
                                x: pos.x,
                                y: pos.y,
                ...
                GameAction::Pass => {
                    if game_view.is_our_turn
                        && game_view.game.apply_move(p2pgo_core::Move::Pass).is_ok()
                    {
                        game_view.is_our_turn = false;
            ...
            TrainingAction::SelectFiles => {
                    ...
                    .pick_files()
                {
                    if paths.len() <= 10 {
                        ...
                        self.training_view.error_message =
                            Some("Please select at most 10 files".to_string());
                    }
                }
            }
        ...
        while let Ok(msg) = self.net_rx.try_recv() {
            match msg {
                ...
                NetToUi::GameList { games } => {
                    self.lobby_view.available_games = games
                        .into_iter()
                        .map(|g| crate::features::lobby_view::GameListing {
                            code: g.id,
