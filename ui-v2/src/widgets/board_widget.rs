...
use p2pgo_core::{Color, Coord, GameState};
...
impl<'a> BoardWidget<'a> {
    ...
    pub fn show(self, ui: &mut Ui) -> BoardResponse {
        ...
        let board_rect =
            Rect::from_center_size(response.rect.center(), Vec2::splat(board_pixel_size));

        // Draw board background
        painter.rect_filled(board_rect.expand(margin / 2.0), 4.0, Colors::BOARD);
    ...
    fn draw_heat_map(
        ...
        for x in 0..9 {
            for y in 0..9 {
                ...
                if value > 0.01 {
                    let pos =
                        self.board_pos_to_screen(Coord::new(x as u8, y as u8), rect, cell_size);
    ...
    fn draw_stone(&self, painter: &Painter, center: Pos2, radius: f32, color: Color) {
        ...
        // Shadow
        painter.circle_filled(center + Vec2::new(1.0, 1.0), radius, shadow_color);
