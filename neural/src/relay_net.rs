...
impl RelayNeuralNet {
    ...
    fn extract_features(&self, state: &NetworkState) -> Vec<f32> {
        ...
        let avg_rtt: f32 =
            state.edges.values().map(|e| e[0]).sum::<f32>() / state.edges.len().max(1) as f32;

        let avg_loss: f32 =
            state.edges.values().map(|e| e[1]).sum::<f32>() / state.edges.len().max(1) as f32;
...
#[cfg(test)]
mod tests {
    use super::{NetworkState, RelayNeuralNet};
    use std::collections::HashMap;
    ...
    fn test_relay_prediction() {
        ...
        state
            .nodes
            .insert("node_a".to_string(), vec![10.0, 100.0, 0.99, 0.2]);
        state
            .nodes
            .insert("node_b".to_string(), vec![15.0, 80.0, 0.95, 0.3]);
        state.edges.insert(
