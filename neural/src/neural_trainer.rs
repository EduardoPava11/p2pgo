...
use crate::{config::NeuralConfig, SimpleNeuralNet, TrainingData};
use anyhow::Result;
use p2pgo_core::{Color, GameState, Move, SGFParser};
...
impl NeuralTrainer {
    ...
    pub async fn train_from_sgf_batch(
        &mut self,
        sgf_paths: &[&Path],
    ) -> Result<BatchTrainingResult> {
    ...
    pub fn get_stats(&self) -> TrainingStats {
        TrainingStats {
            ...
            total_positions: self.training_data.iter().map(|td| td.moves.len()).sum(),
...
impl SimpleNeuralNet {
    ...
    pub fn update_patterns(
        ...
        _state: &GameState,
        _mv: &Move,
        weights: &crate::config::NeuralWeights,
    ) {
