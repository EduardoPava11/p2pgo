...
impl SgfToCborConverter {
    ...
    pub fn convert_batch(&self, sgf_paths: &[&Path], output_path: &Path) -> Result<()> {
        ...
        if all_examples.is_empty() {
            return Err(anyhow!("No valid training examples extracted from SGF files"));
        }
        
        // Use first source as primary, but note this is a batch
        let mut primary_source = sources.into_iter().next()
    ...
    fn process_single_sgf(&self, sgf_path: &Path) -> Result<(TrainingSource, Vec<TrainingExample>)> {
        let sgf_content = fs::read_to_string(sgf_path)?;
    ...
    fn extract_source_info(&self, sgf_content: &str, sgf_path: &Path) -> Result<TrainingSource> {
        ...
        let game_id = sgf_path.file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("unknown")
            .to_string();
            
        // Extract player names and ranks (simplified)
        let black_player = self.extract_property(sgf_content, "PB").unwrap_or_else(|| "Unknown".to_string());
        let white_player = self.extract_property(sgf_content, "PW").unwrap_or_else(|| "Unknown".to_string());
        let black_rank = self.extract_property(sgf_content, "BR").unwrap_or_else(|| "?".to_string());
        let white_rank = self.extract_property(sgf_content, "WR").unwrap_or_else(|| "?".to_string());
        let result = self.extract_property(sgf_content, "RE").unwrap_or_else(|| "?".to_string());
    ...
    fn replay_and_collect(&self, final_state: &GameState, source: &TrainingSource) -> Result<Vec<TrainingExample>> {
    ...
    fn create_batch(&self, source: TrainingSource, examples: Vec<TrainingExample>) -> Result<CBORTrainingBatch> {
