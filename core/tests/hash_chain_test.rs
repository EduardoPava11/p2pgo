...
#[cfg(test)]
mod tests {
    use p2pgo_core::engine::{MoveRecord, calculate_move_hash};
    use blake3;
    ...
    fn create_move_with_hash(prev_hash: Option<[u8; 32]>, coord: (u8, u8), player: u8) -> MoveRecord {
    ...
    fn test_move_record_hash_chain() {
        ...
        for i in 1..moves.len() {
            assert_eq!(moves[i].prev_hash, moves[i-1].broadcast_hash, 
                "Hash chain broken at move {}", i);
        }
    }
    ...
    fn test_calculate_move_hash() {
        ...
        assert_eq!(hash1, hash2, "Hash calculation doesn't match expected result");
    }
    ...
    fn prop_hash_uniqueness() {
        fn test(records: Vec<ArbMoveRecord>) -> bool {
            ...
            let unique_moves: HashSet<_> = records.iter()
                .map(|r| (r.0.x, r.0.y, r.0.player))
                .collect();
            ...
            let hashes: Vec<_> = records.iter()
                .map(|r| r.0.broadcast_hash)
                .collect();
    ...
    fn test_tamper_detection() {
        ...
        assert_ne!(chain.last().unwrap().broadcast_hash, 
        ...
        for i in 1..tampered_chain.len() {
            assert_eq!(tampered_chain[i].prev_hash, tampered_chain[i-1].broadcast_hash,
