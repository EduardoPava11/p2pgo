...
impl SgfProcessor {
    ...
    /// Parse a property
    fn parse_property(&self, chars: &mut std::iter::Peekable<std::str::Chars>) -> Result<SgfProperty> {
        // Parse property ID
    ...
    fn parse_property_value(&self, chars: &mut std::iter::Peekable<std::str::Chars>) -> Result<String> {
        // Expect '['
        if chars.next() != Some('[') {
