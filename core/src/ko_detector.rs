use crate::{board::Board, Color, Coord, Move};
...
impl KoDetector {
    ...
    pub fn process_move(
        ...
        if let (Some(coord), Move::Place { color, .. }) = (move_coord, m) {
            for ko in &mut self.ko_situations {
                if ko.recapture_move.is_none() && coord == ko.ko_point && *color != ko.initiator {
    ...
    fn is_potential_ko(
        ...
        let our_neighbors = neighbors
            .iter()
            .filter(|&&coord| {
                coord.is_valid(board.size())
                    && board.get(coord) == Some(color)
                    && coord != capture_coord
            })
            .count();
    ...
    pub fn get_ko_with_context(&self, moves: &[Move], context_size: usize) -> Vec<KoSituation> {
        self.ko_situations
            ...
            .map(|ko| {
                ...
                let end_idx =
                    (ko.recapture_move.unwrap_or(ko.capture_move) + context_size).min(moves.len());

                let mut ko_with_context = ko.clone();
                ko_with_context.context_moves = moves[start_idx..end_idx]
                    .iter()
                    .map(|m| {
                        match m {
...
impl KoSequenceAnalyzer {
    ...
    pub fn extract_ko_sequences(
        &self,
        ko_situations: &[KoSituation],
        all_moves: &[Move],
    ) -> Vec<KoTrainingSequence> {
        ko_situations
            .iter()
            .filter_map(|ko| {
                ...
                if end > start {
                    let context_moves = all_moves[start..end]
                        .iter()
                        .map(|m| match m {
                            Move::Place { x, y, color } => ContextMove {
                                color: *color,
