...
use crate::{cbor::MoveRecord, GameState, Move};
...
pub fn create_move_record(x: u8, y: u8, player: u8, prev_hash: Option<[u8; 32]>) -> MoveRecord {
    ...
    record.broadcast_hash = Some(calculate_move_hash(
        &record.prev_hash.unwrap_or([0u8; 32]),
        &move_data,
    ));
...
pub fn verify_hash_chain(moves: &[MoveRecord]) -> bool {
    ...
    for i in 1..moves.len() {
        ...
        if moves[i].prev_hash.is_none() || moves[i - 1].broadcast_hash.is_none() {
            println!(
                "Missing hash at position {}: prev_hash={:?}, prev broadcast={:?}",
                i,
                moves[i].prev_hash,
        ...
        if moves[i].prev_hash != moves[i - 1].broadcast_hash {
            println!(
                "Hash mismatch at position {}: prev_hash={:?}, prev broadcast={:?}",
                i,
                moves[i].prev_hash,
                moves[i - 1].broadcast_hash
            );
...
mod tests {
    ...
    fn test_hash_chain_integrity() {
        ...
        for i in 0..5 {
            ...
            println!(
                "Move {}: prev_hash={:?}, broadcast_hash={:?}",
                i, record.prev_hash, record.broadcast_hash
            );
