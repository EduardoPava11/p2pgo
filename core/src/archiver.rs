...
pub fn archive_finished_game(game: &GameState, opponent: &str) -> Result<PathBuf> {
    ...
    let sanitized_opponent =
        opponent.replace(|c: char| !c.is_alphanumeric() && c != '-' && c != '_', "_");
    let filename_base = format!("{}_vs_{}", date, sanitized_opponent);

    // Serialize the game data to CBOR
    let cbor_data = serde_cbor::to_vec(&game).context("Failed to serialize game to CBOR")?;
...
pub fn archive_finished_game_async(game: GameState, opponent: String) {
    ...
    std::thread::spawn(move || match archive_finished_game(&game, &opponent) {
        Ok(path) => {
...
pub fn read_game_archive(path: &PathBuf) -> Result<GameState> {
    ...
    let mut file = std::fs::File::open(path).context("Failed to open game archive file")?;
    ...
    if path.to_string_lossy().ends_with(".gz") {
        ...
        // Parse the CBOR data
        serde_cbor::from_slice(&decompressed).context("Failed to parse decompressed CBOR data")
    } else {
        // Parse directly as CBOR
        serde_cbor::from_slice(&buffer).context("Failed to parse CBOR data")
    }
}
...
mod tests {
    ...
    fn test_archive_compression() {
        ...
        if data_size >= COMPRESSION_THRESHOLD {
            assert!(
                path.to_string_lossy().ends_with(".cbor.gz"),
                "Large data ({} bytes) should be compressed with .cbor.gz extension",
                data_size
            );
        } else {
            assert!(
                path.to_string_lossy().ends_with(".cbor"),
                "Small data ({} bytes) should have .cbor extension",
                data_size
            );
        }
