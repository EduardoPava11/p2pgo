...
impl ValueLabeller {
    ...
    pub fn get_value_label(&self, move_number: u32) -> Option<&ValueLabel> {
        self.move_values
            .iter()
            .find(|v| v.move_number == move_number)
    }
        ...
        for radius in 1..=3 {
            for dx in -radius..=radius {
                for dy in -radius..=radius {
                    ...
                    if check_x >= 0
                        && check_x < game_state.board_size as i8
                        && check_y >= 0
                        && check_y < game_state.board_size as i8
                    {
                        let idx = (check_y as usize) * (game_state.board_size as usize)
                            + (check_x as usize);
