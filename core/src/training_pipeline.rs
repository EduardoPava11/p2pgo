...
impl TrainingPipeline {
    ...
    pub fn export_training_data(&self, output_dir: &Path) -> Result<()> {
        ...
        info!("Exported {} games ({} black wins, {} white wins)", 
    ...
    pub fn train_from_data(&self, data_dir: &Path) -> Result<TrainingResults> {
        ...
        if training_examples.len() < self.config.min_games {
            warn!("Insufficient training data: {} games < {} required", 
                  training_examples.len(), self.config.min_games);
        ...
        info!("Sword examples: {}, Shield examples: {}", 
              sword_examples.len(), shield_examples.len());
        ...
        for epoch in 0..self.config.epochs {
            ...
            if epoch % 5 == 0 {
                info!("Epoch {}/{}: {:?} accuracy = {:.3}", 
                      epoch + 1, self.config.epochs, role, accuracy);
            }
        }
    ...
    pub fn collect_game_data(&self, game_state: &GameState, outcome: GameOutcome) -> Result<()> {
        ...
        info!("Collected training data from game {}. Total examples: {}", 
              game_state.id, total);
