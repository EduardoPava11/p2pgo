...
impl BurnEngine {
    ...
    fn run_wasm_inference(&self, model: &WasmModel, game_state: &GameState) -> Result<Vec<f32>> {
        ...
        let _infer_func = instance.get_typed_func::<(u32, u32), u32>(
            &mut store,
    ...
    pub fn collect_training_data(
        ...
        for (move_idx, mv) in game_state.moves.iter().enumerate() {
            if let Move::Place { x, y, color } = mv {
                let board_cbor =
                    base64::prelude::BASE64_STANDARD.encode(self.encode_board_state(game_state)?);
        ...
        info!(
            "Collected {} training examples from game {}",
    ...
    pub fn export_training_data(&self, output_dir: &Path) -> Result<()> {
        ...
        info!(
            "Exported {} games with {} total examples to {}",
            games_len,
            data.len(),
            output_dir.display()
        );
    ...
    pub fn get_training_stats(&self) -> (usize, usize, usize) {
        ...
        let black_wins = data
            .iter()
            .filter(|d| matches!(d.outcome, GameOutcome::BlackWin))
            .count();
        let white_wins = data
            .iter()
            .filter(|d| matches!(d.outcome, GameOutcome::WhiteWin))
            .count();
