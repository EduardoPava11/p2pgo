...
impl PortManager {
    ...
    pub fn get_relay_port(&self) -> Result<u16> {
        // First check if we have a saved port assignment
        {
            ...
            if let Some(port) = assignment.relay_port {
                ...
                tracing::warn!("Saved relay port {} is not available, picking a new one", port);
            }
        }
    ...
    pub fn get_relay_ports(&self) -> Result<(u16, u16)> {
        ...
        {
            let assignment = self.assignment.lock().unwrap();
            if let (Some(tcp_port), Some(udp_port)) = (assignment.relay_port, assignment.relay_udp_port) {
                // Check if both ports are available
                if is_port_available(tcp_port) && is_udp_port_available(udp_port) {
                    tracing::info!("Using persisted relay ports: TCP={}, UDP={}", tcp_port, udp_port);
        ...
        tracing::info!("Selected new relay ports: TCP={}, UDP={}", tcp_port, udp_port);
    ...
    pub fn get_http_port(&self) -> Result<u16> {
        // First check if we have a saved port assignment
        {
            ...
            if let Some(port) = assignment.http_port {
                ...
                tracing::warn!("Saved HTTP port {} is not available, picking a new one", port);
            }
        }
    ...
    fn save_assignment(&self, assignment: &PortAssignment) -> Result<()> {
        ...
        let toml = toml::to_string_pretty(assignment)
            .context("Failed to serialize port assignment")?;
        
        fs::write(&self.config_path, toml)
            .context("Failed to write port assignment file")?;
...
fn load_port_assignment(config_path: &PathBuf) -> Result<PortAssignment> {
    ...
    toml::from_str::<PortAssignment>(&content)
        .with_context(|| format!("Failed to parse port config file: {}", config_path.display()))
}
...
pub fn create_reusable_socket(port: u16) -> Result<std::net::TcpListener> {
    ...
    #[cfg(all(target_os = "linux", target_env = "gnu"))]
    {
        ...
        if let Err(e) = socket.set_reuse_port(true) {
            tracing::warn!("Failed to set SO_REUSEPORT (usually only supported on Linux kernel ≥3.9): {}", e);
        }
    }
...
pub fn pick_available_port() -> Result<u16> {
    ...
    bail!("No available ports found in range {}-{}", MIN_PORT, MAX_PORT)
}
...
mod tests {
    ...
    fn test_port_persistence() {
        ...
        *second_manager.assignment.lock().unwrap() = assignment;
        ...
        assert_eq!(port, second_port, "Port persistence failed: expected {}, got {}", port, second_port);
    }
