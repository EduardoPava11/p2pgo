...
pub async fn set_snapshot_directory(
    channel: &GameChannel,
...
pub async fn write_snapshot(channel: &GameChannel) -> Result<()> {
    ...
    match tokio::fs::rename(&temp_path, &final_path).await {
        ...
        Err(e) => {
            tracing::warn!(
                "Failed to rename snapshot file: {}. Falling back to copy+delete",
                e
            );
            ...
            if dst_metadata.len() == src_metadata.len() {
                ...
                return Err(anyhow::anyhow!(
                    "Snapshot file copy verification failed: size mismatch"
                ));
            }
        }
    }
...
) -> Result<Option<p2pgo_core::GameState>> {
...
mod tests {
    ...
    async fn test_snapshot_roundtrip() -> Result<()> {
        ...
        game_state.moves.push(p2pgo_core::Move::Place {
            x: 4,
            y: 4,
