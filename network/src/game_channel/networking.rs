...
use {
    ...
    tokio::sync::{broadcast, RwLock, Mutex},
    serde_json,
    blake3,
...
pub async fn subscribe_to_game_topic(channel: &mut GameChannel) -> Result<()> {
    if let Some(iroh_ctx) = &channel.iroh_ctx {
        ...
        tracing::info!("Successfully subscribed to gossip for game: {}", channel.game_id);
    }
    Ok(())
}
...
pub async fn broadcast_move(channel: &GameChannel, record: &MoveRecord) -> Result<()> {
    if let Some(iroh_ctx) = &channel.iroh_ctx {
        ...
        match iroh_ctx.broadcast_move(&channel.game_id, &mut record.clone()).await {
            Ok(()) => {
...
#[cfg(feature = "iroh")]
pub async fn broadcast_move_to_peers(channel: &GameChannel, move_record: &MoveRecord) -> Result<()> {
...
pub async fn connect_to_peer(channel: &GameChannel, peer_ticket: &str) -> Result<()> {
    if let Some(iroh_ctx) = &channel.iroh_ctx {
        ...
        // Add the connection to our peer list
        {
            ...
            tracing::debug!("Added peer connection for game {}, total: {}", channel.game_id, connections.len());
        }
        ...
        tracing::debug!("Successfully connected to peer for game: {}", channel.game_id);
    }
    Ok(())
}
                        ...
                        if let Ok(move_record) = serde_json::from_str::<MoveRecord>(message) {
                            ...
                            if let Err(e) = state::process_received_move_direct(
                                ...
                                if let Some(channel) = channel {
                                    if let Err(e) = send_move_ack(&channel, move_index, Some(&connection)).await {
                            ...
                            let channel = super::GameChannel::get_for_game_id(&sync_req.game_id).await;
                            if let Some(channel) = channel {
                                if let Err(e) = super::sync::handle_sync_request(&channel, sync_req, Some(&connection)).await {
                            ...
                            let channel = super::GameChannel::get_for_game_id(&sync_resp.game_id).await;
                            if let Some(channel) = channel {
                                if let Err(e) = super::sync::handle_sync_response(&channel, sync_resp).await {
        ...
        if let Some(content) = extract_bytes(&event) {
            ...
            match serde_cbor::from_slice::<MoveRecord>(&content) {
                Ok(move_record) => {
                    ...
                    // Process the received move
                    process_received_move_from_gossip(
                        move_record,
                        events_tx,
                        latest_state,
                    ).await?;
...
#[cfg(feature = "iroh")]
pub async fn send_move_ack(channel: &GameChannel, move_index: usize, to_connection: Option<&Connection>) -> Result<()> {
    if let Some(iroh_ctx) = &channel.iroh_ctx {
        tracing::debug!("Sending ACK for move index {} in game {}", move_index, channel.game_id);
