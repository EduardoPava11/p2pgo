...
pub fn verify_move_chain_integrity(move_records: &[MoveRecord]) -> bool {
            ...
            Some(actual_prev_hash) => {
                if actual_prev_hash != &expected_prev_hash {
                    tracing::warn!(
                        ...
                        i, expected_prev_hash, actual_prev_hash
                    );
                    return false;
                }
            }
    ...
    tracing::debug!("Move chain integrity verified for {} moves", move_records.len());
    true
}
...
pub fn verify_game_state_proof(game_state: &p2pgo_core::GameState, expected_proof: &str) -> bool {
    ...
    if matches {
        ...
        tracing::warn!("Game state proof mismatch: expected {}, got {}", expected_proof, actual_proof);
    }
...
mod tests {
    use super::*;
    use p2pgo_core::{GameState, Move, Coord, MoveRecord};
