...
use crate::relay_mesh::{GossipConfig, RelayCapabilities, RelayMesh, RelayNode, RelayStats};
...
impl RelayServer {
    ...
    pub async fn new(config: RelayServerConfig) -> Result<Self> {
        ...
        let socket_addr: SocketAddr = bind_addr.parse().context("Invalid bind address")?;
    ...
    fn start_game_cleanup_task(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // Rebuild index
                let games_snapshot: Vec<_> = store
                    .games
                    .iter()
    ...
    pub async fn store_game(&self, game_data: GameData) -> Result<()> {
        ...
        info!(
            "Storing game {} ({}x{})",
            game_data.id, game_data.board_size, game_data.board_size
        );
        ...
        store
            .index
            .recent
            .push_back((game_data.id.clone(), std::time::Instant::now()));
