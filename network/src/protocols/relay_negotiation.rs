...
impl RelayNegotiationProtocol {
    ...
    pub fn new(our_capacity: RelayCapacity) -> Self {
        let our_relay_offer = RelayOffer {
            ...
            valid_until: (Instant::now() + Duration::from_secs(86400)).elapsed().as_secs(),
        };
    ...
    fn default_priority_tiers() -> Vec<PriorityTier> {
        vec![
            ...
            PriorityTier {
                ...
                requirements: TierRequirements::ReciprocityRatio { 
                    min_ratio: 0.8 
                },
                benefits: TierBenefits {
                    priority: 2,
            ...
            PriorityTier {
                ...
                requirements: TierRequirements::Reputation { 
                    min_score: 0.0 
                },
                benefits: TierBenefits {
                    priority: 1,
    ...
    pub async fn handle_message(
        ...
        match message {
            ...
            NegotiationMessage::AcceptAgreement { offer_hash, counter_offer } => {
                self.handle_acceptance(from, offer_hash, counter_offer).await?;
            }
    ...
    async fn handle_metrics_update(
        &mut self,
        from: PeerId,
            ...
            match &agreement.their_offer.terms.reciprocity {
                ReciprocityRequirement::Balanced { tolerance } => {
                    if ratio < (1.0 - tolerance) || ratio > (1.0 + tolerance) {
                        ...
                        warn!("Suspended relay agreement with {:?} due to reciprocity", peer_id);
                    }
                }
