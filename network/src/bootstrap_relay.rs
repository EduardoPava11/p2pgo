    ...
    pub async fn broadcast_discovery(&mut self) -> Result<()> {
        let discovery_rna = RNAMessage {
            ...
            rna_type: RNAType::RelayDiscovery {
                addresses: self.listening_addresses()
                    .into_iter()
                    .map(|a| a.to_string())
                    .collect(),
        ...
        debug!("Broadcast relay discovery with score: {}", self.discovery_score);
    ...
    async fn handle_event(&mut self, event: SwarmEvent<BootstrapBehaviourEvent>) -> Result<()> {
        match event {
            SwarmEvent::ConnectionEstablished { peer_id, endpoint, .. } => {
                ...
                self.connected_peers.insert(peer_id, PeerInfo {
            ...
            SwarmEvent::Behaviour(BootstrapBehaviourEvent::Gossipsub(gossipsub::Event::Message {
                propagation_source,
                message_id,
                message,
            })) => {
            ...
            SwarmEvent::Behaviour(BootstrapBehaviourEvent::Mdns(mdns::Event::Discovered(peers))) => {
            ...
            SwarmEvent::Behaviour(BootstrapBehaviourEvent::Kademlia(kad::Event::RoutingUpdated {
                ...
                debug!("Kademlia routing updated for {}: {} addresses", peer, addresses.len());
            }
    ...
    async fn handle_rna_message(&mut self, rna: RNAMessage, from: PeerId) -> Result<()> {
        match &rna.rna_type {
            RNAType::SGFData { .. } | RNAType::PatternData { .. } => {
                ...
                info!("Discovery score increased to {} from training data", self.discovery_score);
            }
            RNAType::ModelWeights { consensus_count, .. } => {
