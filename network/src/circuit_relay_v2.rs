...
pub enum RelayMessage {
    ...
    /// Heartbeat/keepalive
    Ping { from: PlayerId, nonce: u64 },

    /// Heartbeat response
    Pong { from: PlayerId, nonce: u64 },
}
...
impl CircuitRelayNode {
    ...
    pub fn handle_message(&mut self, msg: RelayMessage) -> Option<RelayMessage> {
        match msg {
            ...
            RelayMessage::GameMove { from, .. } => self.handle_game_move(from, msg),

            RelayMessage::CreditTransfer {
                from,
                to,
                amount,
                reason,
            ...
            RelayMessage::Ping { from: _, nonce } => Some(RelayMessage::Pong {
                from: self.id,
                nonce,
            }),
    ...
    pub fn cleanup_sessions(&mut self) {
        ...
        self.relay_sessions
            .retain(|_id, session| now.duration_since(session.last_activity) < timeout);
    }
