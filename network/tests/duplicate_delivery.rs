...
use common::test_utils::{
    get_processed_sequences_len, random_move, wait_for_events, was_move_deduplicated,
};
...
async fn test_move_deduplication() -> Result<()> {
    ...
    assert!(
        final_deque_size > initial_deque_size,
        "Deduplication queue should have grown"
    );

    // Verify the deduplication queue stays under limit (8192)
    assert!(
        final_deque_size <= 8192,
        "Deduplication queue should stay under limit"
    );
...
async fn test_dedup_queue_limit() -> Result<()> {
    ...
    assert!(
        queue_size <= 8192,
        "Deduplication queue should stay under limit (got {})",
        queue_size
    );
