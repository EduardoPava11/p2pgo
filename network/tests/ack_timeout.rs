...
async fn test_ack_timeout() -> Result<()> {
    ...
    assert!(sync_requested, "Sync request should have been triggered by the watchdog");
    ...
    assert!(!sync_requested, "No sync request should be triggered when ACK is received");
