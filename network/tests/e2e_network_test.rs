...
async fn test_two_player_game_with_score() -> Result<()> {
    ...
    let board_size = 9;
    let game_id = lobby1.create_game(Some("Player1".to_string()), board_size, false).await?;
    ...
    // Verify game is over
    assert!(game_state.is_game_over(), "Game should be over after two passes");
    ...
    assert!(score_proof.territory_black > 0, "Black should have territory");
    assert!(score_proof.territory_white > 0, "White should have territory");
    ...
    assert_eq!(moves.len(), moves2.len(), "Both players should have the same number of moves");
