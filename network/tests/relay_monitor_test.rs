...
async fn test_restartable_relay() -> Result<()> {
    ...
    assert!(
        matches!(
            status,
            RelayHealthStatus::Restarting | RelayHealthStatus::Healthy
        ),
...
async fn test_restartable_relay_with_panic() -> Result<()> {
    ...
    assert!(
        panic_counter.load(std::sync::atomic::Ordering::SeqCst) > 0,
