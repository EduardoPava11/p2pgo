...
use common::test_utils::{
    get_latest_snapshot, random_move, submit_move, wait_for_events, wait_for_snapshot_update,
};
...
async fn test_snapshot_cadence() -> Result<()> {
    ...
    assert!(
        ...
        "Snapshot should have been updated after moves"
    );
    ...
    let time_based_mtime =
        wait_for_snapshot_update(&game_id, Some(after_moves_mtime), 5000).await?;

    // Verify that the snapshot was updated due to time
    assert!(
        time_based_mtime > after_moves_mtime,
...
async fn test_snapshot_file_integrity() -> Result<()> {
    ...
    assert!(
        snapshot_bytes.len() > 10,
        "Snapshot file should have reasonable size"
    );
