...
#[cfg(feature = "iroh")]
mod tests {
    use p2pgo_network::iroh_endpoint::{EnhancedTicket, IrohCtx};
    ...
    async fn test_enhanced_ticket_roundtrip() {
        ...
        let ticket2 = ctx
            .ticket_with_game_size(Some(9))
            .await
            .expect("Failed to generate ticket with game size");

        // Decode both tickets and verify they're valid
        let bytes1 = B64.decode(&ticket1).expect("Failed to decode base64");
        let decoded1: EnhancedTicket =
            serde_cbor::from_slice(&bytes1).expect("Failed to decode CBOR");

        let bytes2 = B64.decode(&ticket2).expect("Failed to decode base64");
        let decoded2: EnhancedTicket =
            serde_cbor::from_slice(&bytes2).expect("Failed to decode CBOR");
    ...
    fn test_enhanced_ticket_cbor_format() {
        ...
        let decoded: EnhancedTicket =
            serde_cbor::from_slice(&bytes).expect("Failed to decode CBOR");
        ...
        let decoded_ticket: EnhancedTicket =
            serde_cbor::from_slice(&decoded_cbor).expect("Failed to decode CBOR");
