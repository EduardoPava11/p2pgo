...
mod tests {
    ...
    async fn test_multi_board_size_lobbies() {
        ...
        let mut rx_9 = ctx2
            .subscribe_lobby(9)
            .await
            .expect("Failed to subscribe to 9x9 lobby");
        let mut rx_19 = ctx2
            .subscribe_lobby(19)
            .await
            .expect("Failed to subscribe to 19x19 lobby");

        tokio::time::sleep(Duration::from_millis(100)).await;

        // Advertise 19x19 game
        ctx1.advertise_game("test-game-19x19", 19)
            .await
            .expect("Failed to advertise 19x19 game");
        ...
        assert!(
            result_9.is_err(),
            "9x9 lobby should not receive 19x19 advertisement"
        );
