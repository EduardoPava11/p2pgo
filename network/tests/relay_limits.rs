...
use p2pgo_network::relay_monitor::{RestartableRelay, RelayCapacityReport};
...
mod common;
use common::{spawn_relay, TestRelay, RelayConfig, TestPeer, PeerConfig};
...
async fn test_relay_connection_limit_direct() -> Result<()> {
    ...
    let (capacity_tx, mut capacity_rx) = tokio::sync::mpsc::unbounded_channel::<RelayCapacityReport>();
    ...
    for i in 0..300 {
        ...
        match Endpoint::builder()
            ...
            Ok(endpoint) => {
                ...
                match endpoint.connect_with_relays(&[relay_addr]).timeout(Duration::from_millis(500)).await {
                    Ok(Ok(_)) => {
    ...
    assert!(successful_connections <= 210, "Too many connections accepted: {}", successful_connections);
    assert!(successful_connections >= 190, "Too few connections accepted: {}", successful_connections);
    
    // Check the capacity report
    if let Some(report) = capacity_rx.recv().await {
        println!("Received capacity report: {} connections / {} max", 
            report.current_connections, report.max_connections);
            
        assert_eq!(report.max_connections, 200, "Max connections not properly configured");
...
async fn test_relay_connection_limits_with_peers() -> Result<()> {
    ...
    let mut rejected_peer = TestPeer::new(
        PeerConfig {
...
async fn test_relay_bandwidth_limits() -> Result<()> {
    ...
    let bob_state = bob_channel.get_latest_state().await;
    assert!(bob_state.is_some(), "Bob should eventually receive some state despite throttling");
