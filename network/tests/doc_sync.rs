...
mod tests {
    ...
    async fn document_sync_between_nodes() -> Result<()> {
        ...
        node_a.store_training_move(&game_id, 0, test_move_data).await?;
        
        // Get node B's address and explicitly sync with it
        let node_b_addr = node_b.node_addr().await?;
        println!("Explicitly syncing document with node B: {}", node_b_addr.node_id);
        node_a.sync_document_with_node(&game_id, &node_b_addr).await?;
        ...
        assert!(!fetched_moves.is_empty(), "Node B should have received the document");
        println!("Document successfully synchronized, found {} moves", fetched_moves.len());
