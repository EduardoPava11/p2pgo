...
use anyhow::Result;
use p2pgo_network::{Lobby, GameChannel, GameId};
use p2pgo_core::{Move, Coord, Color, GameEvent, MoveRecord};
use std::sync::Arc;
...
async fn test_duplicate_flood() -> Result<()> {
    ...
    assert!(added_entries <= 2, "Too many entries added to dedup queue: {}", added_entries);
