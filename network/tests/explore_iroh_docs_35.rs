...
mod tests {
    ...
    fn explore_available_types() {
        ...
        // Store modules
        println!(
            "store::fs::Store exists: {}",
            std::any::type_name::<iroh_docs::store::fs::Store>()
        );

        // Check for Document - try different paths
        println!(
            "store::Document: {}",
            option_type_name::<Option<iroh_docs::store::Document>>()
        );
        println!(
            "DocDriver: {}",
            option_type_name::<Option<iroh_docs::DocDriver>>()
        );

        // Check for Author
        println!(
            "store::Author: {}",
            option_type_name::<Option<iroh_docs::store::Author>>()
        );

        // Show some high-level exported types
        println!(
            "NamespaceId: {}",
            std::any::type_name::<iroh_docs::NamespaceId>()
        );
