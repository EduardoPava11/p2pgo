...
mod tests {
    ...
    use p2pgo_core::{GameState, Move, Coord, Color, GameEvent};
    use std::sync::Arc;
    ...
    async fn first_stone_appears_on_both_boards() -> Result<()> {
        ...
        let received_event = timeout(Duration::from_secs(10), events_b.recv())
            .await
            .map_err(|_| anyhow::anyhow!("Timeout waiting for first stone to appear on player B's board"))?
        ...
        let state_a = channel_a.get_latest_state().await
            .ok_or_else(|| anyhow::anyhow!("Channel A has no state"))?;
        let state_b = channel_b.get_latest_state().await
        ...
        assert_eq!(stone_a, stone_b, "Both boards should have the same stone at (4, 4)");
    ...
    async fn multiple_stones_sync_correctly() -> Result<()> {
        ...
        for mv in &moves {
            if let Move::Place(coord) = mv {
                ...
                let stone_a = state_a.board[idx];
                let stone_b = state_b.board[idx];
                assert_eq!(stone_a, stone_b, "Stone at {:?} should match on both boards", coord);
