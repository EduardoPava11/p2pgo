...
async fn test_duplicate_move_deduplication() -> Result<()> {
    ...
    let board_size = 9;
    let game_id = alice_lobby.create_game(Some("Alice".to_string()), board_size, false).await?;
    ...
    assert_eq!(bob_moves.len(), initial_move_count + 1, "Bob should have received one move");
    assert_eq!(bob_moves.last().unwrap(), &test_move, "Bob should have received the correct move");
    ...
    assert_ne!(hash_after_first_delivery, initial_hash, "Game state should have changed after move");
    ...
    bob_channel.handle_duplicate_move_test(duplicate_record).await?;
    ...
    assert_eq!(
        hash_after_duplicate,
        hash_after_first_delivery,
