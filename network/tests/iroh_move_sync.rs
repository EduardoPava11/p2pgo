...
#[cfg(feature = "iroh")]
mod tests {
    use p2pgo_network::{iroh_endpoint::IrohCtx, game_channel::GameChannel};
    ...
    async fn test_move_sync_between_channels() {
        ...
        let channel1 = GameChannel::with_iroh(
            ...
            initial_state.clone(), 
            Arc::new(ctx1)
        ).await.expect("Failed to create first game channel");
        
        let channel2 = GameChannel::with_iroh(
            game_id.clone(), 
            initial_state.clone(), 
            Arc::new(ctx2)
        ).await.expect("Failed to create second game channel");
        ...
        channel1.send_move(test_move.clone()).await.expect("Failed to send move from channel1");
        ...
        let state1 = channel1.get_latest_state().await.expect("Failed to get state from channel1");
        let state2 = channel2.get_latest_state().await.expect("Failed to get state from channel2");
    ...
    async fn test_doc_replay_functionality() {
        ...
        for mv in &moves {
            channel1.send_move(mv.clone()).await.expect("Failed to send move");
    ...
    async fn test_gossip_and_doc_consistency() {
        ...
        for (i, mv) in test_moves.iter().enumerate() {
            if i % 2 == 0 {
                channel1.send_move(mv.clone()).await.expect("Failed to send move from channel1");
            } else {
                channel2.send_move(mv.clone()).await.expect("Failed to send move from channel2");
            }
        ...
        for (i, expected_move) in test_moves.iter().enumerate() {
            assert_eq!(&moves1[i], expected_move, "Channel1 move {} should match", i);
            assert_eq!(&moves2[i], expected_move, "Channel2 move {} should match", i);
        }
    }
}
