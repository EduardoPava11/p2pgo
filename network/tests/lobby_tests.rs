...
async fn test_lobby_create_and_list_games() {
    ...
    // Create a few games
    let game1 = lobby
        .create_game(Some("Game 1".to_string()), 9, false)
        .await
        .unwrap();
    let game2 = lobby
        .create_game(Some("Game 2".to_string()), 13, false)
        .await
        .unwrap();
    let game3 = lobby
        .create_game(Some("Game 3".to_string()), 19, true)
        .await
        .unwrap();
...
async fn test_lobby_events() {
    ...
    // Create a game
    let game_id = lobby
        .create_game(Some("Event Test Game".to_string()), 9, false)
        .await
        .unwrap();
...
async fn test_lobby_game_interaction() {
    use p2pgo_core::{Color, Coord, GameEvent, Move};
