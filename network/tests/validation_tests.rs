...
mod tests {
    ...
    fn mk_connected_blob(id: &str, prev_blob: &MoveBlob, prev_hash: [u8; 32], seq: u32) -> MoveBlob {
    ...
    fn blob_validates_moves() {
        ...
        assert!(b1.validate_continuation(Some(&gs1), Some(b0.hash())).is_err());
    }
    ...
    fn chain_enforces_consistency() {
        ...
        assert!(chain.add_blob(other_blob).is_err(), "Expected error for wrong game ID");
        
        // Invalid: disconnected sequence
        let b2 = mk_connected_blob("test-chain", &b0, b0.hash(), 2); // skips 1
        assert!(chain.add_blob(b2).is_err(), "Expected error for skipped sequence");
