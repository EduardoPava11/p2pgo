...
async fn test_relay_config_custom() -> Result<()> {
    ...
    match loaded_config.relay_mode {
        RelayModeConfig::Custom => {
            assert_eq!(
                loaded_config.relay_addrs,
                vec!["https://custom-relay.example.com".to_string()]
            );
        }
...
async fn test_config_loading() -> Result<()> {
    ...
    assert!(
        !config.relay_addrs.is_empty(),
        "Config should have relay addresses"
    );
    assert!(
        config.gossip_buffer_size > 0,
        "Config should have positive gossip buffer size"
    );
...
async fn test_relay_health_events() -> Result<()> {
    use p2pgo_network::relay_monitor::{RelayHealthEvent, RelayHealthStatus};
    use tokio::sync::mpsc;
    ...
    let event1 = timeout(Duration::from_secs(1), health_rx.recv())
        .await?
        .unwrap();
    let event2 = timeout(Duration::from_secs(1), health_rx.recv())
        .await?
        .unwrap();
    ...
    assert_eq!(
        ...
        "First event should be Restarting"
    );
    assert_eq!(
        event2.status,
        p2pgo_network::relay_monitor::RelayHealthStatus::Healthy,
        "Second event should be Healthy"
    );
