...
async fn test_scoring_agreement_between_nodes() -> Result<()> {
    ...
    let channel1 =
        GameChannel::with_iroh(game_id.clone(), initial_state.clone(), iroh_ctx1.clone()).await?;
    ...
    let channel2 =
        GameChannel::with_iroh(game_id.clone(), initial_state.clone(), iroh_ctx2.clone()).await?;
    ...
    let state1 = channel1
        .get_latest_state()
        .await
        .expect("Channel 1 should have a game state");
    let state2 = channel2
        .get_latest_state()
        .await
        .expect("Channel 2 should have a game state");
    ...
    assert_eq!(
        score1.territory_black, score2.territory_black,
        "Black territory should match"
    );
    assert_eq!(
        score1.territory_white, score2.territory_white,
        "White territory should match"
    );

    // Simulate accepting the score and sharing with opponent via iroh
    channel1
        .send_event(GameEvent::GameFinished {
    ...
    assert!(
        received_game_finished,
        "Player 2 should have received game finished event"
    );
