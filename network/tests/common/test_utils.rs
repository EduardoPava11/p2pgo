...
use p2pgo_core::{Move, Coord, GameState, GameEvent, Color, MoveRecord};
...
pub async fn was_move_deduplicated(
    ...
    let result = timeout(Duration::from_millis(timeout_ms), async {
        loop {
            match rx.recv().await {
                Ok(GameEvent::MoveMade { mv: ref event_move, .. }) => {
                    if event_move == mv {
                        if seen {
...
pub async fn submit_move(channel: &Arc<GameChannel>, mv: Move) -> Result<()> {
    ...
    channel.handle_local_move(mv).await
        ...
        // Use the process_move method that takes a MoveRecord
        #[cfg(feature = "iroh")]
        {
            // We need to call the process_network_move method
            // Use a direct function pattern for testing
            let game_id = channel.get_game_id().to_string();
            tracing::debug!("Test: Processing move for game {}: {:?}", game_id, record);
            
            // Direct call using standard API - handle_local_move is public
            channel.handle_local_move(mv).await?;
        ...
        #[cfg(not(feature = "iroh"))]
        {
            return Err(anyhow::anyhow!("Not implemented for non-iroh build"));
        }
    }
    
    Err(anyhow::anyhow!("No game state available"))
}
