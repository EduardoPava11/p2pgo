...
mod tests {
    ...
    async fn test_self_relay_bootstrap() -> Result<()> {
        ...
        let relay_addr = format!(
            "/ip4/127.0.0.1/tcp/{}/quic-v1/p2p/{}",
            relay_port, self_node_id
        );
        ...
        assert_ne!(
            self_node_id, client_node_id,
            "Client and relay should have different node IDs"
        );
        ...
        assert!(
            relay_connectivity.is_online,
            "Local relay should be reachable"
        );
    ...
    async fn test_relay_restart() -> Result<()> {
        ...
        assert_eq!(
            node_id, new_node_id,
            "Node ID should remain the same after restart"
        );
        ...
        assert!(
            relay_connectivity.is_online,
            "Local relay should be reachable after restart"
        );
