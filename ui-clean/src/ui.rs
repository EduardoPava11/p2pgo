...
use eframe::egui::{self, Color32, RichText, Vec2, Pos2, Rect, Stroke};
...
impl P2PGoApp {
    fn render_header(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            // Navigation
            if ui.selectable_label(self.view == View::MainMenu, "Menu").clicked() {
                self.view = View::MainMenu;
            }
            
            if ui.selectable_label(self.view == View::Settings, "Settings").clicked() {
                self.view = View::Settings;
            }
            
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ui.add_space(20.0);
                ui.label(format!("Heat Map: {} (H)", if self.show_heat_map { "🔥" } else { "❄️" }));
                ui.separator();
        ...
        ui.vertical_centered(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Start Game (Debug)").clicked() {
                    self.view = View::Game { game_id: game_id.to_string() };
                }
            });
        });
    }
    ...
    fn render_board(&mut self, ui: &mut egui::Ui, game_state: &p2pgo_core::GameState, game_id: &str) {
        let size = 600.0;
        ...
        let margin = 30.0;
        let board_rect = Rect::from_min_size(rect.min + Vec2::splat(margin), Vec2::splat(size - 2.0 * margin));
            ...
            for x in 0..board_size {
                ...
                if idx < game_state.board.len() {
                    if let Some(color) = &game_state.board[idx] {
                        ...
                        match color {
                            GoColor::Black => painter.circle_filled(pos, cell_size * 0.45, Color32::BLACK),
                            GoColor::White => {
                                painter.circle_filled(pos, cell_size * 0.45, Color32::WHITE);
                                painter.circle_stroke(pos, cell_size * 0.45, Stroke::new(1.0, Color32::BLACK));
                            }
                        }
                    }
                }
