...
use p2pgo_network::relay_provider::{RelayProviderFactory, PlayerGuild, RelayPreferences};
...
fn main() {
    ...
    for player_count in 2..=5 {
        ...
        println!("  {} players -> {} relay mode", player_count, provider.name());
    }
    ...
    println!("    -> Using {} for low-latency direct connection", provider.name());
    ...
    println!("    -> Using {} for triangular relay with credit incentives", provider.name());
    
    // Scenario 3: Large tournament
    println!("\n  Scenario: 8-player tournament");
    let provider = RelayProviderFactory::create_provider(8, vec![]);
    println!("    -> Using {} for scalable multi-player support", provider.name());
